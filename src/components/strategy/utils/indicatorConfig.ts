export interface IndicatorParameter {
  name: string;
  type: 'number' | 'dropdown' | 'checkbox_group' | 'radio_button' | 'boolean' | 'string' | 'time';
  label: string;
  default: any;
...
export interface Indicator {
  ...
  outputs?: string[]; // For multi-output indicators, specify available output parameters
}
...
// Timeframe options for strategy configuration
export const timeframeOptions = [
  '1m', '3m', '5m', '15m', '30m', '1h', '2h', '4h', '1d', '1w', '1M'
];

// Exchange options for strategy configuration
...
export const indicatorConfig: IndicatorConfig = {
  "HLSCAN": {
    "function_name": "HLSCAN",
    "display_name": "High-Low Scanner",
    "description": "Displays the highest and lowest prices within a selected time window, useful for intraday breakout strategies and range analysis.",
    "parameters": [
      {
        "name": "start_time",
        "type": "time",
        "label": "Start Time",
        "default": "09:15",
        "description": "Start time of the range window (HH:MM, 24-hour format)"
      },
      {
        "name": "end_time",
        "type": "time",
        "label": "End Time",
        "default": "10:30",
        "description": "End time of the range window (HH:MM, 24-hour format)"
      }
    ],
    "outputs": ["Range High", "Range Low"]
  },
  "SMA": {
    "function_name": "SMA",
