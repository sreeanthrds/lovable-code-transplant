...
const TOAST_LIMIT = 3
const TOAST_REMOVE_DELAY = 2000 // 2 seconds for quick auto-dismiss
...
function toast({ title, description, variant, ...props }: Toast) {
  // Create a key to identify duplicate toasts
  const toastKey = `${title || ''}-${description || ''}-${variant || 'default'}`
  const now = Date.now()
  
  // Check if we recently showed the same toast
